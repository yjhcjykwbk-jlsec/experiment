...
final class CameraConfigurationManager {
	...
	private static Point getCameraResolution(Camera.Parameters parameters,
		...
		if (previewSizeValueString != null) {
			...
			// Point tempResolution =
			// findBestPreviewSizeValue(previewSizeValueString,
			// screenResolution);
			// cameraResolution = new Point(
			// (tempResolution.x >> 3) << 3,
			// (tempResolution.y >> 3) << 3);
		...
		if (cameraResolution == null) {
			...
			// screen may not be.
			cameraResolution = new Point((screenResolution.y >> 3) << 3,
					(screenResolution.x >> 3) << 3);
		}

		Log.d(TAG, "cameraResolution is:" + cameraResolution.x + ","
				+ cameraResolution.y);
	...
	private static Point findBestPreviewSizeValue(
		...
		if (bestX > 0 && bestY > 0) {
			Log.d(TAG, "best point is:" + bestX + "," + bestY);
