...
////////////////////
  ...
  public  ProxyServer(ServerAuthenticator auth,Socket s,File file,String digest,Socks5Send send){
      this.auth  = auth;
      this.sock  = s;
      this.file = file;
      this.send = send;
      ...
      try{
        ...
        while(true){
          ...
          log("Accepted from:"+s.getInetAddress().getHostName()+":"
                              +s.getPort());
          ProxyServer ps = new ProxyServer(auth,s,null,null,null);
          (new Thread(ps)).start();
        }
        ...
        case ACCEPT_MODE:
          try{
            ...
            //pipe_thread1.interrupt(); //Tell other thread that connection have
                                      //been accepted.
            System.out.println("accept pipe");
           // pipe(remote_in,out);
          }catch(IOException ioe){
...
//         s = new SocksSocket(proxy,msg.ip,msg.port);
      ...
      System.out.println(msg.host);
      
     if(msg.host.equals(sha1Digest) && msg.port==0)
     {
		 log("Connected to "+msg.host+":"+msg.port);
...
//////////////////
  ...
  //       if(pipe_thread1 != null) pipe_thread1.interrupt();
   ...
   private void pipe(OutputStream out) throws IOException
	{
