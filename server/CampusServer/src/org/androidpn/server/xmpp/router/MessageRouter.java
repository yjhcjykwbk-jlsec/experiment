...
public class MessageRouter {
	...
	/**
	 * Routes the Message packet. many times the Router will give the packet to
	 * handlers, such as persenseUpdateHandler
	 * 
	 * @param packet
	 *            the packet to route
	 */
	public void route(Message packet) {
		...
		if (!isOL(packet.getFrom())) {
			...
			// @todo
			log.info("sending message to the recver:" + packet.getTo());
			ClientSession rSession = sessionManager.getSession(packet
					.getTo()+"");//here parameter should be string type, since it is not JID
			if (rSession != null) {
				try{
				log.info(rSession.toString()+":"+rSession.getHostName()+rSession.getHostAddress());
				rSession.process(packet);
				}catch(Exception e){
					e.printStackTrace();
				}
			} else
